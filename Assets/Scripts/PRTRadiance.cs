...
[ExecuteAlways]
public class PRTRadiance : ScriptableRendererFeature
{
    class CustomRenderPass : ScriptableRenderPass
    {
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            cmd.name = "PRTRadiance";


            // 如果存在 probe volume, 需要 clear 一下探针组的球谐系数 buffer
            ProbeVolume[] volumes = FindObjectsOfType<ProbeVolume>();
            ...
            //dispatch
            Probe[] probes = FindObjectsOfType<Probe>();
            foreach (var probe in probes)
            {
                if (probe == null) continue;
                probe.TryInit();
                probe.SurfelRadiance(cmd);
            }
